...
const singleBed = computed(() => {
  ...
  return props.room?.beds?.filter((bed: BedType) => bed === "SINGLE").length || 0
})

const doubleBed = computed(() => {
  return props.room?.beds?.filter((bed: BedType) => bed === "DOUBLE").length || 0
})
...
<template>
  <Card v-if="props.room" class=" w-full  md:w-[300px] shadow-2xl rounded-t-2xl  border-2"
    ...
    <template #content>
      <div class="text-color flex flex-col">
        ...
        <p class="mt-0">
          <small class="text-sm mr-2">Cena:</small>
          <strong>{{ UtilsService.formatCurrency(props.room.price) }}/mc</strong>
        </p>
      </div>
    </template>
    <template #footer>
      <small class="text-sm">Info: </small>
        <p id="info">{{props.room.info}}</p>
    </template>
  </Card>
</template>
